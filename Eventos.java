    ...
    public class DownloadFoto extends AsyncTask<Void, Void, Bitmap[]> {
        ...
        protected Bitmap[] doInBackground(Void... params) {
            ...
            for(int i = 0; i < listaImagensEvento.length; i++){
                try{
                    //faz o download da foto, arredonda os cantos e armazena em um array temporario
                    imagens[i] = getRoundCornerBitmap(Picasso.with(Eventos.this).load(listaImagensEvento[i]).resize(150, 150).get(), 20);
                } catch (Exception e){
                    e.printStackTrace();
                }
            }
